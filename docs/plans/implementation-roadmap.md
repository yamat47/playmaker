...
### 実装メモ：フォーメーションテンプレート（preset + 外部受け入れ + 読込配線）（2026-05-17, M6）

PRD 5.6 を VSCode 流の軽量適用で実装。判断ロジックを common に寄せて node 単体で
全網羅し、browser は薄い DOM シェルに保った。M5 で繰り延べた「フォーメーション読込」
（5.4 の 1 項目）をここで確定した。

**M6 で確定（common: formations・commands・テスト済 / 229 tests・common 100%・v8 ignore 0）**
- `Formation` = 名前付き選手テンプレート集合。`FormationPlayer = Omit<Player,"id">`（id は
  読込時に `IdFactory` で採番＝既存選手・Undo 復活と衝突しない）。`side`(`offense`/`defense`)
  は **UI グルーピング用メタ**で配置ロジックに影響させず、Model に side 概念を持ち込まない
  （PRD 4.1 表現範囲を絞る）
- **読込セマンティクス = マージ（追記）**。既存の選手・線を保持しテンプレート選手を追記。
  PRD 5.6「攻守の代表的隊形を読み込んで自動配置」をそのまま満たし、攻守プリセットを順に
  重ねられる。replace でなく add ゆえ **単一可逆コマンド**（新規 id で従属線を持たない＝
  `AddPlayerCommand` と同じ理屈・直線履歴 LIFO で undo のメメント破棄が安全）
- **M4 契約準拠**：`LoadFormationCommand`（common/commands）。Model に原子バッチ
  `addPlayers`/`removePlayers` を追加し、`addPlayerCore`/`removePlayerCore` を抽出して
  「単発＝単一 emit」を保ったまま一括も **1 回だけ emit**（1 コマンド = onChange 1 回）。
  `removePlayerCore` のカスケード除去・dangling 防止・未知 id throw はバッチへそのまま継承
- **外部受け入れ（PRD 6.6）**：`normalizeFormation` が `normalizePlayers` を再利用
  （位置不正は除外・shape/label 既定補完）、id を落として `FormationPlayer` 化、配置可能な
  選手 0 人なら null、id/name/side は欠落・不正でも既定補完。正規化は公開境界
  （`Playmaker.loadFormation`）で行い `resolvePlayData` と同じ層配置に揃える
- プリセット = 攻 2（I フォーメーション / ショットガン スプレッド）・守 2（4-3 / ニッケル）
  各 11 人、middle ゾーン窓（abs 35..65）に収まる座標。`FORMATION_PRESETS` /
  `getFormationPreset`（未知 id は undefined）

**M6 で確定（browser/公開: 最小限・VRT なし）**
- `EditorController.loadFormation`（`IEditorController`）= 唯一の編集入口。`IdFactory` で
  既存と衝突しない id を採番（バッチ内重複も `taken` に積んで回避）、配置可能な選手が無ければ
  no-op、読込後は stale 防止で選択解除。`Playmaker.loadFormation` を公開（外部隊形を
  正規化→controller。view モードでも API としては有効）
- `Toolbar` に攻守 optgroup の `<select>`（controller 経由＝Undo/onChange の対象、選択後は
  プレースホルダへ戻し追記を続けられる、`view` は CSS で非表示）。未知 id ガードは公開
  `getFormationPreset` に委譲（再実装しない）。demo は内蔵 select ＋公開 API ボタンで
  「クリア→攻→守」の追記を目視

**M8 / M9 へ繰り延べ**
- `onChange` の往復契約（version/migration 含む確定）= **M8**
- 見た目磨き（プリセット座標の戦術精緻化）= **M9**
- `IPlayModel` に深いクローンを伴わない軽量 id 取得 API（`loadFormation`/`addPlayerAt`/
  `lineIds` が `getData()` 全複製→id 抽出を共有）。現規模で実害なし・既存 M5 と同一
  イディオムのため 3 箇所一括で = **M9 仕上げ**

### 実装メモ：PNG エクスポート（編集 UI 除外）（2026-05-17, M7）

PRD 5.7 を VSCode 流の軽量適用で実装。寸法決定（純計算）を common に寄せて node
単体で全網羅し、Canvas/`toBlob` 依存の薄い殻のみ browser に置いた。

**M7 で確定（common: export・テスト済 / 240 tests・common 100%・v8 ignore 0）**
- `resolveImageExportSize`（`common/export`）= 外部由来のエクスポート設定 → 出力 px
  寸法。`width?` 1 ノブ（高さは `FIELD_WINDOW_ASPECT` から導出）。`resolvePlayData`
  / `normalizeFormation` と同じ公開境界の防御正規化作法で、非有限・1 未満は
  `DEFAULT_EXPORT_WIDTH` へ丸める（投げない）。`width>=1 ⇒ height>=1` が成り立つ
  ので冗長な下限ガードを置かず common 100% を素直に維持
- アスペクト比はゾーン非依存（どの窓も縦 30yd）。出力寸法をこの比に合わせるため
  エクスポート画像はレターボックス余白なし（`FieldGeometry` の offset がほぼ 0）

**M7 で確定（browser/公開: 最小限・VRT なし）**
- `CanvasSurface` から **プレー本体合成 `drawPlay`（field→lines→players・overlay 無し）**
  を抽出し、ライブ `render()` と新 `exportToPngBlob` が同一構図を共有。これで
  「export = 画面 − 編集 UI」を**構造で保証**（選択強調・waypoint ハンドル・作図中
  プレビューは drawPlay が描かない＝構造的に非対象。ツールバー/パネルは HTML 兄弟
  要素で canvas 外＝元から非対象）
- `exportToPngBlob(data, options?)` = オフスクリーン canvas を寸法分作り `drawPlay`
  →`toBlob("image/png")`。配色は画面と同じ host の `--playmaker-*` を読むので
  オンスクリーンと一致。`Playmaker.exportToPng` は **コミット済み `model.getData()`**
  を渡す（作図中プレビューも除外＝「現在の保存済みプレー」を出力。view/edit 両対応）
- demo は「PNG を出力」ボタンでダウンロードし、編集 UI 非含有・view/edit 同一図を目視

**M8 / M9 へ繰り延べ**
- `onChange` の往復契約（version/migration 含む確定）= **M8**
- `scale`/DPR・透過背景・余白指定・出力体裁の磨き = **M9**
