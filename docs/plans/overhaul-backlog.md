...
8. ユーザーに尋ねるべき論点が無ければ、確認を挟まずに `/create-pr` で PR を作る（`.claude/rules/workflow.md` の確認より優先する。2026-09-23 にユーザーが指示）。マージはユーザーが行う。
...
- T15c ブラウザテスト（done）。Browser Mode を入れ、Docker イメージにも Chromium を入れる。Chromium は Dockerfile と CI の両方で、lockfile の playwright と同じ版の `playwright install --with-deps chromium` で入れる（2026-09-23 に決着）。Debian の chromium は版が playwright の想定とずれるので使わない。cloud 版のセッションでは `/opt/pw-browsers` の Chromium を環境変数で指す。
  - 結論: Vitest の projects で node と browser を分け、`make test-browser` と CI の `browser-test` ジョブで動かす。`make check` にも入れた。テストは `src/playmaker.browser.test.ts` の 19 本で、DOM の組み立てと後始末、ポインタとキー、ツールバー、パネル、テーマ変数の読み直し、setPlayData のあとの付け直し、PNG の書き出しを確かめる。playwright は package.json で版を固定し、Dockerfile はその版を読んで Chromium を入れる。cloud 版のセッションでは `CHROMIUM_PATH` で `/opt/pw-browsers/chromium` を指す。テストの iframe が Playwright のページに収まらないと Vitest が縮めて映し、ドラッグの起点だけが縮めた座標で押されたので、viewport をページに収まる大きさにした。
...
- 進み具合: T15-0、T15a、T15b1〜T15b4、T15c、T15-8、T15-9 の 8 本で閉じた。T15-3 と T15-4 は editor-controller.test.ts ごと消えた。
