...
- T15b 仕様テストへの移行。機能のまとまりごとに 3 本に割る。届かないと分かったコードの削除は別コミットにする。下の T15-1〜T15-7 はここで片付くか、消える。
  - T15b-1 選手の編集（追加、選択、ドラッグ、削除、パネル、上限）（done）。更新コマンドが無い id を自分で探して throw していた分岐は、公開の入口から届かないので、PlayModel の requirePlayer と requireLine に寄せた。
  - T15b-2 線の編集（作図、waypoint と終点のハンドル、削除、パネル、上限）。interaction と preview のテストもここで消す。line-commands.test.ts にある、渡したパッチをあとで書き換えても当てる値が変わらないテストは、仕様テストに置き換えてから消す。
  - T15b-3 履歴、ゾーン、隊形の読み込み、通知と、残りのコマンドとサービスのテスト。editor-controller.test.ts をここで消し、残る単体テストの T15-5〜T15-7 も片付ける。
