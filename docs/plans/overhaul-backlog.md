...
- [x] **T8-1 [should] UndoRedoService と CommandService がどちらも model を持ち、apply の経路が 2 本ある。履歴イベントもない**（T1-1 の恒久策）
  ...
  - 結論: IUndoRedoService は peekUndo、peekRedo、push、markUndone、markRedone だけを持つ。ICommandService は履歴の変化を onDidChangeHistory で流す。EditorController は IUndoRedoService に依存しなくなり、履歴には ICommandService を通してだけ触る。T1-1 の「発火を編集の後へ遅らせる」仕組みは、T9 でイベントを分けるまで残した。
