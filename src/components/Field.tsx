...
interface FieldProps {
  ...
  onLineSelect?: (lineId: string | null, lines?: Line[]) => void;
  startRouteDrawing?: {
    playerId?: string;
    lineId?: string;
...
const Field = ({
    ...
    if (startRouteDrawing) {
      ...
      if (startRouteDrawing.playerId) {
        ...
        if (player) {
          ...
          onPlayerSelect?.(null);
          onLineSelect?.(newLine.id, [...lines, newLine]);
        ...
        if (
          ...
          if (pointInfo) {
            ...
            if (segment && pointInfo.localIndex < segment.points.length) {
              ...
              setSelectedPoint(null);
              onLineSelect?.(startRouteDrawing.lineId, updatedLines);
  ...
  useEffect(() => {
    ...
    const isPathEqual = (path1: number[], path2: number[] | null): boolean => {
